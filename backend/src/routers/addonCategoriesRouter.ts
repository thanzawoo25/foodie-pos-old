...
addonCategoriesRouter.put(
  ...
  async (request: Request, response: Response) => {
    const addonCategoryId = request.params.id;
    const isValid = addonCategoryId && request.body.name;
    ...
    const hasExistingAddonCategories = existingAddonCategories.rows.length > 0;
    if (!hasExistingAddonCategories) return response.send(400);
